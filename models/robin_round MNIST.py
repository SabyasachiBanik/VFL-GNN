...
def load_mnist_vertically_partitioned_robin_round(num_clients=2, augment=True):
    ...
    def split_images_robin_round(data):
        """Split images in robin-round fashion across ROWS (height dimension)"""
        splits = []
        for client_id in range(num_clients):
            # Select every num_clients-th ROW starting from client_id
            # Rows: client_id, client_id + num_clients, client_id + 2*num_clients, ...
            client_rows = data[:, :, client_id::num_clients, :]  # Changed from [:, :, :, client_id::num_clients]
            splits.append(client_rows)
        return splits
...
#def normalize_data(data, mean=0.2860, std=0.3530): # for FMNIST 
def normalize_data(data, mean=0.1307, std=0.3081):
    """Normalize data with MNIST statistics"""
...
# SPATIAL ADJACENCY INITIALIZATION (IMP)
def create_spatial_prior_adjacency(height, width, sigma=2.0):
    """Create adjacency matrix with spatial priors for actual client dimensions"""
...
class EnhancedClientModel(nn.Module):
    def __init__(self, input_shape, gnn_hidden_dims=[64, 32], mlp_hidden=64, num_classes=10, sparsity_reg=1e-4):
        ...
        c, h, w = input_shape  # w is client-specific width after robin-round split
        self.flatten_dim = h * w  # Different for each client
        self.height = h
        self.width = w
        self.sparsity_reg = sparsity_reg

        # Initialize adjacency with spatial priors using client-specific dimensions
...
def train_one_round(client_models, client_optimizers, client_schedulers, client_loaders, round_idx, consensus=True):
    ...
    # Consensus phase - only for clients with same flatten_dim
    if consensus and round_idx > warmup_rounds:
        current_alpha = adaptive_consensus_rate(round_idx - warmup_rounds, α, 0.1)
        
        # Group clients by their flatten_dim
        dim_groups = {}
        for k in range(K):
            dim = client_models[k].flatten_dim
            if dim not in dim_groups:
                dim_groups[dim] = []
            dim_groups[dim].append(k)
        
        # Apply consensus within each group
        for dim, client_indices in dim_groups.items():
            if len(client_indices) > 1:  # Only apply consensus if multiple clients in group
                W_updates = [client_models[k].W.clone().detach() for k in client_indices]
                
                for idx, k in enumerate(client_indices):
                    # Find neighbors within the same dimension group
                    neighbors_in_group = [client_indices[(idx - 1) % len(client_indices)], 
                                         client_indices[(idx + 1) % len(client_indices)]]
                    
                    consensus_W = (1 - current_alpha) * W_updates[idx]
                    for n_idx, n in enumerate(neighbors_in_group):
                        n_pos = client_indices.index(n)
                        consensus_W += (current_alpha / len(neighbors_in_group)) * W_updates[n_pos]
...
# SINGLE SEED EXPERIMENT (Multi-seeds later)
def run_experiment_single_seed(num_clients, seed, verbose=False):
    ...
    # Model initialization - each client gets model based on their actual data shape
    client_models = []
    for k in range(num_clients):
        input_shape = train_splits[k].shape[1:]  # (C, H, W_k) where W_k varies
        model = EnhancedClientModel(input_shape, gnn_hidden_dims=[64, 32], mlp_hidden=64).to(device)
